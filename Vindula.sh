...
vercionaDor(){
    ...
    versConf=$(curl ufpr.dl.sourceforge.net/project/vindula/2.0.3/scripts/IntranetVindula_BETA.sh \
        | sed -r '/^# Vers/!g' \
        | sed '/^$/d' \
        | sed -n '/\#/{p;q;}' \
        | sed 's/-.*$//'\
...
atualizaDor(){
	...
	wget -cq "http://ufpr.dl.sourceforge.net/project/vindula/2.0.3/scripts/IntranetVindula_BETA.sh"

	linhaVersoa=$(cat $(basename "$0") | sed -n '/^versaoAtual/{=;d;}')

	echo "$(sed ''$linhaVersoa's/.*/'$versConfC'/' < $(basename "$0"))" > $(basename "$0")

	chmod +x IntranetVindula_BETA.sh
...
"
 ...
 if [[ -f IntranetVindula_BETA.sh ]]; then 
